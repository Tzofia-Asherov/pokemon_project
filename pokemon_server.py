...
def get_pokemons_details():
    ...
    if type_name:
        ...
        return json.dumps({"pokemons by "+ type_name : pokemons_id_list})

    return json.dumps({"error" : "no key trainer_name/pokemon_name/type_name"}), 400
...
def add_new_trainer():
    ...
    if trainer_name == None:
        return json.dumps({"error" : "no key name trainer_name"}), 400 
    town_name = response["town_name"]
    if town_name== None:
        return json.dumps({"error" : "no key name town_name"}), 400 
    town_id = queries.get_town_id_by_its_name(town_name)
    if town_id== None:
        return json.dumps({"error" : "no "+ town_name + " in db"}), 400 
    queries.add_trainer(trainer_name, town_id)
    return json.dumps({"added new trainer" : trainer_name})
...
def delete_ownership():
    ...
    if pokemon_name== None:
        return json.dumps({"error" : "no key name pokemon_name"}), 400 

    queries.delete_pokemon_trainer(trainer_name, pokemon_name)
    return json.dumps({"delete pokemon "+pokemon_name+ " in trainer" : trainer_name}),204
...
def evolve():
    ...
    if chain_item["evolves_to"] != []:
        ...
        return json.dumps({"warning" : "no where to evolve"}), 200

        
    res = queries.evolve_pokemon_species(pokemon_name, trainer_name, evolve_name)
    if res:
        return json.dumps({"evolve pokemon "+pokemon_name : evolve_name })
    return json.dumps({"error" : "no relation between the pokemon: " + pokemon_name + " and the trainer " + trainer_name}), 404
